...
torch.nn: Neural network module providing various neural network layers and functionalities.
torch.nn.functional: Module providing functional interface to operations in torch.nn.
tqdm: Library for creating progress bars in loops and other iterative processes.
...
class Net(nn.Module):
    ...
    def forward(self, x):
        """Defines the computation performed at every call thriugh computation graph
...
def GetModelSummary(model):
    """Prints the summary of the model which incudes parameters count
...
def Training(model, device, num_epochs, train_loader, test_loader, optimizer, criterion):
    """Trains and tests the model for a defined number of epochs.
...
def GetTrainingTestPlots(train_losses, test_losses, train_acc, test_acc):
    """Produces and presents plots illustrating training and test losses along with accuracy.
